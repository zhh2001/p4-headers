...
header quic_t {
    ...
    bit<variable> dc_id;             // Destination Connection ID                         目标连接ID (长度由 dcil 决定)
    bit<variable> sc_id;             // Source Connection ID                              源连接ID (长度由 scil 决定)
    ...
    bit<variable> dc_id;          // Destination Connection ID  目标连接 ID (长度由连接建立时协商)
