...
header igmp_t {
    ...
    bit<8>  max_response_time;  // Maximum Response Time (in units of 1/10 second, only valid in query messages)     最大响应时间 (单位：1/10秒，仅在查询报文中有效)
    bit<16> checksum;           // Checksum (covers the entire IGMP message)                                         校验和 (覆盖整个 IGMP 报文)
    bit<32> group_address;      // Multicast Group Address (0 in query messages, target multicast group in reports)  组播组地址 (在查询报文中为 0，报告报文中为目标组播组)
}
